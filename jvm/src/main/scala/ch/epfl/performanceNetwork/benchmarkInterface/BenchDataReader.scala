...
class BenchDataReader(
    ...
    val groupEnd: String) {
  @volatile private var allData: Seq[BenchCommitData] = Nil
  def readData(data: String) = {
    val filesLines = data.split(testSeparator)
    val parser = BenchDataParser(parameters)
  ...
  private object BenchDataParser {
    def apply(header: String) = {
      val paramMap = header.split("\\s+").zipWithIndex.map { case (s, i) => s -> i }(collection.breakOut): Map[String, Int]
      val paramPos =
        keyParameters
          .map {
            s =>
    ...
    def parseResult(s: String, paramSeparator: String, completeFormat: (String, String, String)) = {
      ...
      val regexp =
        if (paramSeparator.length() == 1 && completeFormat._2.length() == 1 && completeFormat._2 != paramSeparator)
          ...
          /*
           *  Cut only on separator that arn't between the beginning and the end of a group.
           *  The Begining and end can be escaped with a \ just before. 
           */
