...
object Knapsack {
  ...
  def onMouseDown = (event: MouseEvent) => {
    val (x,y) = (event.layerX.intValue(), event.layerY.intValue())
  ...
  def clearCircle(x: Number, y:Number, r:Number, fillStyle:String ="#FFF") = {
    ctx.beginPath()
    ctx.moveTo(x.doubleValue()+r.doubleValue(),y)
  ...
  def drawText(x:Number, y:Number, text:String = "", fillStyle:String ="#F00") = {
    ctx.fillStyle = fillStyle
