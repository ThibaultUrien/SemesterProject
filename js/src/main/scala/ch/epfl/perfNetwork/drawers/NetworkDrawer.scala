...
class NetworkDrawer(
    ...
    val linkColor: String) extends Drawer {

  /**
   * @param n : The network to be drawn, visiblePoints of this network will also be updated by this method.
   * @param perf : A PerfBarChart, used do draw the link between a commit node and the associated bars.
   * @param v : A View that will provide the offset from the absolute coordinate of point to display coordinate.
   */
  def draw(n: Network, perf: PerfBarChart, v: View): Unit = {
    def colors(i: Int) = n.colors(i)
    def drawArrowHead(
      start: (Double, Double),
      dir: (Double, Double),
      color: String): Unit = {
    ...
    def linkToPerf(commit: Vertex, v: View) = {
    ...
    n.visiblePoints = lastVisiblePoints
